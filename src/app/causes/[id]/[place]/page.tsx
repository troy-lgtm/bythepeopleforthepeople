...
export async function generateMetadata({
  ...
  return {
    title: `${cause.name} in ${placeLabel}: what government is doing`,
    description: cause.description,
    alternates: { canonical: `/causes/${cause.slug}/${place.toLowerCase()}` },
  };
}

/** Place-scoped catalog cause page, e.g. /causes/homelessness/la. */
export default async function CausePlacePage({ params }: PageProps) {
  const { id, place } = await params;
  const cause = getCatalogCause(id);
  const resolved = resolvePlaceParam(place);
